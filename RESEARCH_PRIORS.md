...
- Pretrained YOLO baseline first: before any fine-tuning, `train.py` evaluates the active football YOLO26 path on SynLoc validation through official `mAP-LocSim`. A historical Soccana row exists in the ledger, but Soccana is retired from active defaults. The first run produced thousands of detections but only microscopic `mAP-LocSim` with `recall_50=0.0`, so the loop must debug model loading/class mapping/projection/eval plumbing before training.
